...
num_epochs = 2
batch_size = 1040  # Incease batch size, so # of batches is integer
total = 51891840 
...
model = nn.Sequential(
    nn.Linear(num_inp, num_neurons),
    nn.Sigmoid(),
    nn.Linear(num_neurons, num_neurons),
    nn.Sigmoid(),
    nn.Linear(num_neurons, num_neurons),
    nn.Sigmoid(),
    nn.Linear(num_neurons, num_out),
).to(device)
...
for epoch in range(num_epochs):
    ...
    if shuffle:
        ...
        xtrain_tensor = xtrain_tensor[rand_indx, :]
        ytrain_tensor = ytrain_tensor[rand_indx, :]
        
    #for inputs, targets in dataloader:
    ## manual batching
    for j in range(num_batches):
        inputs = xtrain_tensor[batch_size*j:batch_size*(j+1),:]
        targets = ytrain_tensor[batch_size*j:batch_size*(j+1),:]
        outputs = model(inputs)
        loss = criterion(
            outputs, targets.view(-1, 1)
        )  # -1 - use 1st dimension as in original aray; 1 - second dimension must be 1
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        inputs_count += 1
...
dt_sec = time.time() - t0
print("done! elapsed time (minutes): %.1f" % (dt_sec / 60.0))
